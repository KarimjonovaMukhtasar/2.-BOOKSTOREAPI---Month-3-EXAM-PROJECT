...
const userUpdate = z.object({
    username: z.string().unique().min(5, `TOO SHORT FOR A USERNAME`).max(10, `TOO LONG FOR A USERNAME`).optional(),
    password: z.string().min(6, `TOO SHORT FOR A PASSWORD`).max(15, `TOO LONG FOR A PASSWORD`).optional(),
    email: z.string().email().trim().toLowerCase().optional(),
    first_name: z.string().trim().min(2, `TOO SHORT FOR A FIRSTNAME`).max(20, `TOO LONG FOR  A LASTNAME`).optional(),
    last_name: z.string().trim().min(2, `TOO SHORT FOR A LASTNAME`).max(20, `TOO LONG FOR A LASTNAME`).optional(),
    phone_number: z.string().regex(/^\+\d{10,15}$/, 'Invalid phone number').optional(),
    address: z.string().min(10, `TOO SHORT FOR AN ADDRESS`).max(40, 'TOO LONG FOR AN ADDRESS').optional()
})
